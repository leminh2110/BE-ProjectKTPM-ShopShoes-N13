...
public class WebClientConfig {
    ...
    // Filter để thực hiện retry
    private ExchangeFilterFunction retryFilter() {
        return (request, next) -> next.exchange(request)
                .retryWhen(Retry.backoff(5, Duration.ofMillis(500)) // Retry 5 lần, delay 0.5s
                        .filter(throwable -> {
                            // Chỉ retry cho các lỗi network hoặc server error
                            if (throwable instanceof WebClientResponseException) {
                                WebClientResponseException ex = (WebClientResponseException) throwable;
                                return ex.getStatusCode().is5xxServerError() || 
                                       ex.getStatusCode().value() == 503;
                            }
                            return true; // Retry cho tất cả các lỗi khác
                        })
